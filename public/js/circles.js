...
(function () {
  ...
  Circles.prototype = {
    ...
    generate: function generate(radius) {
      if (radius) {
        this._radius = radius;
    ...
    _confirmAnimation: function _confirmAnimation(duration) {
      if (duration === null) {
        this._canAnimate = false;
        return;
      }
    ...
    _getNumberElement: function _getNumberElement() {
    ...
    _generateText: function _generateText() {
      var html = '<div class="' + this._textWrpClass + '" style="position:absolute; top:0; left:0; text-align:center; width:100%;' + ' font-size:' + this._radius * .7 + 'px; height:' + this._svgSize + 'px; line-height:' + this._svgSize + 'px;">' + this._calculateNumber(this._canAnimate ? 0 : this._number);
      if (this._text) {
    ...
    _calculateNumber: function _calculateNumber(number) {
    ...
    _generateSvg: function _generateSvg() {
      return '<svg width="' + this._svgSize + '" height="' + this._svgSize + '">' + this._generatePath(100, false, this._colors[0]) + this._generatePath(this._canAnimate ? 1 : this._percentage, true, this._colors[1]) + '</svg>';
    },

    _generatePath: function _generatePath(percentage, open, color) {
